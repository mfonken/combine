...
void SYSIOCTL_Init( sys_component_t * list, uint8_t len )
{
    memset( &SysIOCtl, 0, sizeof(SysIOCtl) );
    for( uint8_t i = 0; i < len; i++ ) SYSIOCTL_Push_Component( list[i] );
}

sys_component_t * SYSIOCTL_Get_Component( component_id id )
{
    for( uint8_t i = 0; i < SYSIOCTL_NUM_FAMILIES; i++ )
    {
        for( uint8_t j = 0; j < SysIOCtl.tables.index[i]; j++ )
        {
            sys_component_t * curr = &SysIOCtl.tables.component[i][j];
...
void SYSIOCTL_Push_Component( sys_component_t component )
{
...
void SYSIOCTL_Enable_Family( sys_family_t family )
{
...
void SYSIOCTL_Disable_Family( sys_family_t family )
{
    for( uint8_t i = 0; i < SysIOCtl.tables.index[family]; i++)
    {
...
void SYSIOCTL_Enable_Component( sys_component_t component )
{
    GPIO_Set( component.pin, component.port, 1 );
}

void SYSIOCTL_Disable_Component( sys_component_t component )
{
