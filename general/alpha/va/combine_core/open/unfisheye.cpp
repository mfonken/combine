...
static void init(int width, int height, double zoom, double strength)
{
    Fisheye.half_width = width >> 1;
    Fisheye.half_height = height >> 1;
    Fisheye.zoom = zoom;
    Fisheye.strength = strength;
    
#ifdef QUICK
    Fisheye.correction = (1 / qdist(abs(Fisheye.half_width), abs(Fisheye.half_height))) * Fisheye.strength;
#else
    Fisheye.correction = sqrt((Fisheye.half_width*Fisheye.half_width)+(Fisheye.half_height*Fisheye.half_height)) / Fisheye.strength;
#endif
}

static void unfisheyePoint(point2_t a, point2_t b)
{
#ifdef QUICK
#else
    Fisheye.radius = sqrt((a.x*xn)+(yn*yn)) * correctionRadius_;
