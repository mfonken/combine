...
inline void frameProcessor( void )
{
	while(!frame_flag);
	zero_memory();
	...
	frame_start();
	while(!row_flag);
	//uint32_t wr_index = 0, row_sum = 0;
	//while(frame_flag)
	for( uint32_t y = 1, p = 0; y < CAPTURE_HEIGHT; y++ )
	{
		while(row_flag);
		row_int();
		//tog();
		...
		//row_sum = 0;
		bool r = 1;
		while( ++hcounter < CAPTURE_WIDTH && !row_flag && r )
		{
			r = pixel_proc();
