...
void Kinetic_Update_Position( LSM9DS1_t * imu, kinetic_t * kinetics, cartesian2_t beacons[2] )
{
    ...
    x_B = ( beacons[0].x - CAMERA_HALF_WIDTH  ) * PIXEL_TO_UNIT;
    x_A = ( beacons[1].x - CAMERA_HALF_WIDTH  ) * PIXEL_TO_UNIT;
    y_B = ( beacons[0].y - CAMERA_HALF_HEIGHT ) * PIXEL_TO_UNIT;
    y_A = ( beacons[1].y - CAMERA_HALF_HEIGHT ) * PIXEL_TO_UNIT;
    d_x = x_A - x_B;
    d_y = y_A - y_B;
