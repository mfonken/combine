...
void Init(rho_c_utility * utility, int w, int h)
{
    ...
    utility->density_map_pair.x.variance[0] = 0;
    utility->density_map_pair.x.variance[1] = 0;
    ...
    utility->density_map_pair.y.variance[0] = 0;
    utility->density_map_pair.y.variance[1] = 0;
...
void Filter_and_Select( rho_c_utility * utility, DensityMapC * d, DensityMapC * b, PredictionC * r )
{
    ...
    int range[2] = { b->centroid, 0 };
    int x1 = l, x2 = l;
    for( int i = 0; i < 2; i++ )
    {
        for(; x1 > range[i]; --x1, c = d->map[x1], b1 = b->map[x1] )
        {
            c = abs(c - b1);
            if( c < d->length/8 )
            {
        ...
        d->max[i] = m[i];
        t = d->kalmans[i].value;
        v = RHO_VARIANCE_NORMAL * ( 1 + RHO_VARIANCE_SCALE * ( RHO_K_TARGET - d->kalmans[i].K[0] ) );
        d->variance[i] = v;
        if( v < t && v > 0) g = t - v;
        else return;
        /* * * * * * * * * * * * * * */
