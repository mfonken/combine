...
{ /* Detect regions - START */
    for( _->x = _->start - 1, _->x_ = (_->offset == 0) * _->x + _->offset, _->curr = 0; _->x >= _->end; _->curr = density_map->map[--_->x], _->x_-- ) // TODO: Don't use end for x_ here
    {
//        printf("[%d]\n", _->x);
