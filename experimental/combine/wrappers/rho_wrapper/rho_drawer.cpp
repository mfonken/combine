...
//    LOG_RDR(DEBUG_2, "Initializing Rho Drawer.\n");
    ...
    y_match_data = new int[height];
    y_peak_data_l = new double[height];
    y_peak_data_u = new double[height];
    y_peak_data_lv = new double[height];
    y_peak_data_uv = new double[height];
    y_peak_data_lt = new double[height];
    y_peak_data_ut = new double[height];
...
Point DimPt(int x, int y, dimension_t dim)
{
    return dim == X_DIMENSION ? Point(x, y) : Point(y, x);
}

void DrawDensityMap(Mat& M, density_map_t * dmap, int range[3], int height, dimension_t dim )
{
    int l = range[0];
    ...
    for( int i = 0, j = 1; i < 2; i++, j++ )
    {
        ...
        line(M, DimPt(range[i], mv2, dim), DimPt(range[j], mv2, dim), yellowish);
        ...
        {
            --x;
            u = INR(OP_ALIGN((fX[x]/DENSITY_SCALE), height), height);
            v = INR(OP_ALIGN((dX[x]/DENSITY_SCALE), height), height);

            vv = bX[x]/DENSITY_SCALE;
//            bool vvt = vv>=0;
            vv = INR(OP_ALIGN(abs(vv), height), height);

            if(v > mdm)
            {
//                M.at<Vec3b>(y2,v) = white;
                line(M, DimPt(x, vprev, dim), DimPt(x-1, v, dim), white);
//                line(M, Point(y2, vvprev), Point(y2-1, vv), (vvt?green:red));
            ...
            {
                int under = RANGE(mdm + abs(mdm-v), height-1);
                int over  = RANGE(mdm - abs(mdm-v), height-1);
                if(dim == X_DIMENSION)
                {
