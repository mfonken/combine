...
{
    return ( task_data->tmr != NULL && task_data->period != 0 );
}
...
void OSInterface_CreateTask( os_task_data_t * task_data )
{
    ...
    LOG_OSI(OSI_DEBUG, "Creating OS Task: %s", SYSTEM_TASK_ID_STRINGS[(uint8_t)task_data->ID]);
...
{
    LOG_OSI(OSI_DEBUG, "Resuming OS Task: %s", SYSTEM_TASK_ID_STRINGS[(uint8_t)task_data->ID]);
...
{
    LOG_OSI(OSI_DEBUG, "Suspending OS Task: %s", SYSTEM_TASK_ID_STRINGS[(uint8_t)task_data->ID]);
...
{
    LOG_OSI(OSI_DEBUG, "Deleting OS Task: %s", SYSTEM_TASK_ID_STRINGS[(uint8_t)task_data->ID]);
...
{
    LOG_OSI(OSI_DEBUG, "Creating OS Queue: %s\n", SYSTEM_QUEUE_ID_STRINGS[queue_data->ID]);
