...
//    LOG_KINETIC(KINETIC_LOG_PRIO, "x:%d | y:%d | φ:%d\n", (int)A_x, (int)A_y, (int)(k->e.y*RAD_TO_DEG));
...
//    LOG_KINETIC(KINETIC_LOG_PRIO, "xx:%.2f xy:%.2f xz:%.2f yz:%.2f yy:%.2f yz:%.2f zx:%.2f zy:%.2f zz:%.2f\n", ra.ii, ra.ij, ra.ik, ra.ji, ra.jj, ra.jk, ra.ki, ra.kj, ra.kk);
    ang3_t a;
...
//    n = ra.ij/2;//1.0 - ( 2 * m );
    ...
    {
        k->mu = acos( n );
//        LOG_KINETIC(KINETIC_LOG_PRIO, "x>x %.2f %.2fdeg\n", ra.ii, k->mu*RAD_TO_DEG);
        return 1;
    }
